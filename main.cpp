...
EstimatedTime MultiHeadSelfAttention<T>::forward(T* hidden_states, bool verify, const filesystem::path& data_dir)
{
    ...
    if (verify) {
        size_t attn_count = _batch_size * _num_heads * _seq_len * _seq_len;
        float* ref = new float[attn_count];
        read_binary_file((data_dir / "attn_probs").generic_string(), ref);
        compare_tensors(attn_scores, ref, attn_count, "attn_probs");
        delete[] ref;
    }
